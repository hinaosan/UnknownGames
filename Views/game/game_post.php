...
$Err = [];
$game_name = $category = $play_time = $system = $spec = $story = $sale = "";
function h($str){
...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
  ...
  if (empty($_FILES['image']['tmp_name'])) {
    $Err["image"] = "サムネイル画像は必須です。ファイルを選択してください。";
  }
      ...
      <form method="post" name="post" action="" enctype="multipart/form-data">
        ...
        <div class="koumoku">
          ...
          <?php if (isset($Err["game_name"])): ?>
            ...
            <input type="text" name="game_name" placeholder="" class="c-form" value="<?php echo $game_name;?>">
          </div>
        <?php else: ?>
          <input type="text" name="game_name" placeholder="" class="c-form" value="<?php echo $game_name;?>">
        </div>
    ...
    <?php endif; ?>
      ...
      <div class="koumoku">
        ...
        <?php if (isset($Err["spec"])): ?>
          ...
          <textarea name="spec" placeholder="" class="r-form"><?php echo $spec;?></textarea>
        </div>
      <?php else: ?>
        <textarea name="spec" placeholder="" class="r-form"><?php echo $spec;?></textarea>
      </div>
...
<div class="img-edit">
  <p>サムネイル画像を選択する<Br>(縦:横の比率は1:2～2:3推奨)</p>
